...
class Thresholds
{
    ...
    {
        $optimizeFor = strtolower((string) ($conf->optimize_for ?? 'f1'));
        $targetRecall = $conf->strict_recall_at ?? 0.8;
        $costFP = isset($conf->cost_false_positive) ? (float) $conf->cost_false_positive : 1.0;
        $costFN = isset($conf->cost_false_negative) ? (float) $conf->cost_false_negative : 1.0;
        $beta = isset($conf->beta) ? max(0.0, (float) $conf->beta) : 1.0;
        $betaSq = $beta * $beta;
        $eps = 1e-9;
        ...
        foreach ($candidates as $idx => $threshold) {
            ...
            $point = self::formatPoint($threshold, $cm, $costFP, $costFN, $betaSq);
            $point['threshold_index'] = $idx;
            $points[] = $point;
        }

        if (empty($points)) {
            $fallback = self::formatPoint(0.5, Metrics::confusion($proba, $y, 0.5), $costFP, $costFN, $betaSq);
        ...
        } elseif ($optimizeFor === 'cost') {
            foreach ($points as $point) {
                if (
                    $point['expected_cost'] < $primary['expected_cost'] - $eps ||
                    (abs($point['expected_cost'] - $primary['expected_cost']) <= $eps && (
                        $point['recall'] > $primary['recall'] + $eps ||
                        (abs($point['recall'] - $primary['recall']) <= $eps && $point['precision'] > $primary['precision'] + $eps)
                    ))
                ) {
                    $primary = $point;
                }
            }
        } elseif ($optimizeFor === 'f_beta') {
            foreach ($points as $point) {
                if (
                    $point['f_beta'] > $primary['f_beta'] + $eps ||
                    (abs($point['f_beta'] - $primary['f_beta']) <= $eps && $point['precision'] > $primary['precision'] + $eps)
                ) {
                    $primary = $point;
                }
            }
    ...
    private static function formatPoint(float $threshold, array $cm, float $costFP, float $costFN, float $betaSq): array
    {
        ...
        return [
            ...
            'recall' => $recall,
            'f1' => $cm['f1'],
            'expected_cost' => $costFP * $cm['FP'] + $costFN * $cm['FN'],
            'f_beta' => $den > 0.0 ? ((1 + $betaSq) * $precision * $recall / $den) : 0.0,
            'support' => [
                'tp' => $cm['TP'],
                'fp' => $cm['FP'],
                'tn' => $cm['TN'],
