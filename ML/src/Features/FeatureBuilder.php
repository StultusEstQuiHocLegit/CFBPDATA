...
class FeatureBuilder
{
    ...
    private const DOMAIN_BOUNDS = [
        ...
        'DaysINV' => [0.0, 400.0],
        'DaysAP' => [0.0, 400.0],
        'CashConversionCycle' => [-400.0, 400.0],
        'DividendOmission' => [0.0, 1.0],
        'DebtIssuanceSpike' => [0.0, 1.0],
        'DebtRepaymentSpike' => [0.0, 1.0],
        'leverage_profitability' => [-5.0, 5.0],
        'liquidity_cashflow' => [-2.0, 2.0],
        'size_profitability' => [-5.0, 5.0],
        'leverage_margin' => [-10.0, 10.0],
        'liquidity_accruals' => [-5.0, 5.0],
    ];

    private const TREND_BOUNDS = [-1.0, 1.0];

    private const VOLATILITY_BOUNDS = [0.0, 2.0];
    ...
    public function build(DataFrame $df): DataFrame
    {
        ...
        $trendTargets = self::normaliseDerivedFeatures($trendNames, '_tren');
        $volatilityTargets = self::normaliseDerivedFeatures($volatilityNames, '_vola');
        $trendBases = array_values(array_unique(array_merge(array_keys($trendTargets), array_keys($volatilityTargets))));
        ...
        foreach ($trendTargets as $targetName) {
            if (!isset($features[$targetName])) {
                $features[$targetName] = array_fill(0, count($rows), null);
            }
        }
        foreach ($volatilityTargets as $targetName) {
            if (!isset($features[$targetName])) {
                $features[$targetName] = array_fill(0, count($rows), null);
            }
        }
        ...
        foreach ($companyGroups as $cid => $indices) {
            ...
            foreach ($trendBases as $baseName) {
                if (!isset($features[$baseName])) {
                    continue;
                }
                $trendKey = $trendTargets[$baseName] ?? null;
                $volKey = $volatilityTargets[$baseName] ?? null;
                $prevValue = null;
                $window = [];
                foreach ($indices as $idx) {
                    $current = self::toFloat($features[$baseName][$idx] ?? null);
                    if ($trendKey !== null && isset($features[$trendKey])) {
                        $features[$trendKey][$idx] = ($prevValue !== null && $current !== null) ? ($current - $prevValue) : null;
                    }
                    if ($current !== null) {
                        $window[] = $current;
                        if (count($window) > 3) {
                    ...
                    if ($volKey !== null && isset($features[$volKey])) {
        ...
        self::applyDomainBounds($features, $trendTargets, $volatilityTargets);
    ...
    private static function applyDomainBounds(array &$features, array $trendTargets, array $volatilityTargets): void
    {
        $bounds = self::DOMAIN_BOUNDS;
        foreach ($trendTargets as $targetName) {
            $bounds[$targetName] = self::TREND_BOUNDS;
        }
        foreach ($volatilityTargets as $targetName) {
            $bounds[$targetName] = self::VOLATILITY_BOUNDS;
        }
        foreach ($bounds as $name => [$low, $high]) {
    ...
    // Normalise derived feature declarations such as trends/volatility
    // @param string[] $names
    // @return array<string, string> Map of base feature name -> derived column name
    private static function normaliseDerivedFeatures(array $names, string $suffix): array
    {
        $result = [];
        ...
        foreach ($names as $name) {
            ...
            if ($suffixLength > 0 && strlen($trimmed) >= $suffixLength && substr($trimmed, -$suffixLength) === $suffix) {
                $base = substr($trimmed, 0, -$suffixLength);
                $column = $trimmed;
            } else {
                $base = $trimmed;
                $column = $trimmed . $suffix;
            }
            if ($base === '') {
                continue;
            }
