...
class Isotonic
{
    ...
    public static function fit(array $proba, array $y): self
    {
        ...
        [$thresholds, $values] = self::deduplicateSteps($thresholds, $values);
    ...
    // Collapse duplicate threshold values while keeping the last fitted value
    // @param float[] $thresholds
    // @param float[] $values
    // @return array{0: float[], 1: float[]}
    private static function deduplicateSteps(array $thresholds, array $values): array
    {
        if (empty($thresholds)) {
        ...
        foreach ($thresholds as $idx => $t) {
            if ($last !== null && abs($t - $last) < 1e-12) {
                $filteredValues[count($filteredValues) - 1] = $values[$idx];
                continue;
            }
            $filteredThresholds[] = $t;
            $filteredValues[] = $values[$idx];
            $last = $t;
        }
        return [$filteredThresholds, $filteredValues];
    }
}
