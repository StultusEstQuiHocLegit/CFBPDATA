...
class TrainController
{
    ...
    public function handle(array $post, array $files): Json
    {
        ...
        $trainingResult = Trainer::train($Xtr, $ytr, $Xva, $yva, $weights, $cfg->model);
        ...
        } elseif ($calibrationType !== 'isotonic') {
            return Json::error('Unknown calibration type: ' . $cfg->calibration);
        }
        // @var Isotonic|Platt $calibrator
        ...
        $formatPoint = static function (float $threshold, array $cm, ?int $index = null) {
            $point = [
                'threshold' => $threshold,
        ...
        if (($trainedHyper['model_type'] ?? '') === 'gradient_boosting') {
            $metrics['num_trees'] = $trainedHyper['num_trees'] ?? null;
            $metrics['learning_rate'] = $trainedHyper['learning_rate'] ?? null;
            if (isset($trainedHyper['max_depth'])) {
                $metrics['max_depth'] = $trainedHyper['max_depth'];
            }
        }
