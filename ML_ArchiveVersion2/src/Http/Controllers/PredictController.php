...
class PredictController
{
    ...
    public function handle(array $post, array $files): Json
    {
        ...
        try {
            ...
            $modelType = $meta['hyperparameters']['model_type'] ?? ($meta['config']['model']['type'] ?? 'logistic_regression');
            if ($modelType === 'random_forest') {
                $model = RandomForest::load($modelsDir . '/model.bin');
            } elseif ($modelType === 'gradient_boosting') {
                $model = GradientBoosting::load($modelsDir . '/model.bin');
            } else {
                $model = LogisticRegression::load($modelsDir . '/model.bin');
            }
            $pre = Preprocessor::load($modelsDir . '/preprocessor.bin');
            $calibrationClass = $meta['calibration']['class'] ?? Isotonic::class;
            if ($calibrationClass === Platt::class) {
                $cal = Platt::load($modelsDir . '/calibrator.bin');
            } else {
                $cal = Isotonic::load($modelsDir . '/calibrator.bin');
            }
            $metrics = json_decode(file_get_contents($reportsDir . '/metrics.json'), true);
            $th = $metrics['thresholds'] ?? ['primary' => 0.5, 'recall_target' => 0.5];
        } catch (\Throwable $e) {
        ...
        // Assign risk buckets
        $results = [];
        $bestT = $th['primary'] ?? $th['best'] ?? 0.5;
        $recT = $th['recall_target'] ?? ($th['recall80'] ?? $bestT);
