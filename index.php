<?php
// index.php
// Single file app that reads CSVs and renders modern UI.
...
$datasetMode = (isset($_COOKIE['dataset_mode']) && $_COOKIE['dataset_mode'] === 'subset') ? 'subset' : 'whole';
$suffix = $datasetMode === 'subset' ? '_subset' : '';

$main = array_map(function($r){ $r['solvent'] = false; return $r; }, read_csv_assoc(__DIR__ . '/main' . $suffix . '.csv'));
$financials = array_map(function($r){ $r['solvent'] = false; return $r; }, read_csv_assoc(__DIR__ . '/financials' . $suffix . '.csv'));
$reports = array_map(function($r){ $r['solvent'] = false; return $r; }, read_csv_assoc(__DIR__ . '/reports' . $suffix . '.csv'));

$main_solvent = array_map(function($r){ $r['solvent'] = true; return $r; }, read_csv_assoc(__DIR__ . '/main_solvent' . $suffix . '.csv'));
$financials_solvent = array_map(function($r){ $r['solvent'] = true; return $r; }, read_csv_assoc(__DIR__ . '/financials_solvent' . $suffix . '.csv'));
$reports_solvent = array_map(function($r){ $r['solvent'] = true; return $r; }, read_csv_assoc(__DIR__ . '/reports_solvent' . $suffix . '.csv'));
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .fin-item .na{ opacity:0.5; font-style:italic }
...
<body>
  <button id="dataset-toggle" class="swap-btn" data-mode="<?php echo $datasetMode; ?>" title="<?php echo $datasetMode === 'subset' ? 'currently you are working with the subset, click to switch to the whole dataset' : 'currently you are working with the whole dataset, click to focus on the subset'; ?>"><?php echo $datasetMode === 'subset' ? '◰' : '□'; ?></button>
  <header>
    <div class="wrap">
...
<?php if ($datasetMode === 'subset'): ?>
            <span id="status1" class="status-static" data-value="bankrupt"><span class="dot red"></span><span>bankrupt</span></span>
<?php else: ?>
...
<?php if ($datasetMode === 'subset'): ?>
            <span id="year1" class="year-static" data-value="2024">2024</span>
<?php else: ?>
...
<?php if ($datasetMode === 'subset'): ?>
            <span id="year2" class="year-static" data-value="2024">2024</span>
<?php else: ?>
...
<?php endif; ?>
          ...
          <div class="compare-group">
<?php if ($datasetMode === 'subset'): ?>
            <span id="status2" class="status-static" data-value="solvent"><span class="dot green"></span><span>solvent</span></span>
<?php else: ?>
...
<?php endif; ?>
            <span class="sep-text">|</span>
<?php if ($datasetMode === 'subset'): ?>
            <span id="year3" class="year-static" data-value="2024">2024</span>
<?php else: ?>
...
<?php endif; ?>
            <span class="to-text">to</span>
<?php if ($datasetMode === 'subset'): ?>
            <span id="year4" class="year-static" data-value="2024">2024</span>
<?php else: ?>
...
<?php endif; ?>
  ...
  <script>
    ...
    const DATASET_MODE = <?php echo json_encode($datasetMode, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES); ?>;
    const datasetToggle = document.getElementById('dataset-toggle');
    if (datasetToggle) {
      datasetToggle.addEventListener('click', () => {
        const current = datasetToggle.getAttribute('data-mode') === 'subset' ? 'subset' : 'whole';
        const next = current === 'subset' ? 'whole' : 'subset';
        document.cookie = 'dataset_mode=' + next + '; max-age=' + (30*24*60*60) + '; path=/';
        location.reload();
      });
    }
    ...
    function averageFinancials(status, yStart, yEnd){
      ...
      if(DATASET_MODE === 'subset'){
        [
          ...
          'AIExpectedLikelihoodOfBankruptcyExtendedStrongerModelGPTExplanation',
          'AIExpectedLikelihoodOfBankruptcyBaseStrongerModelGROKExplanation',
          'AIExpectedLikelihoodOfBankruptcyExtendedStrongerModelGROKExplanation',
          ...
          'AIExpectedLikelihoodOfBankruptcyAnnualReportMoreCharactersExplanation'
        ].forEach(key => skip.add(key));
      }
    ...
    function doCompare(){
      ...
      for(const k of keys){
        const entryA = Object.prototype.hasOwnProperty.call(a, k) ? a[k] : null;
        const entryB = Object.prototype.hasOwnProperty.call(b, k) ? b[k] : null;
        const hasA = entryA && Number.isFinite(entryA.mean);
        const hasB = entryB && Number.isFinite(entryB.mean);
        if(!hasA && !hasB) continue;
        const unit = Object.prototype.hasOwnProperty.call(METRIC_UNITS, k) ? METRIC_UNITS[k] : 'USD';
        const v1 = hasA ? Math.round(entryA.mean) : null;
        const v2 = hasB ? Math.round(entryB.mean) : null;
        const med1 = hasA && Number.isFinite(entryA.median) ? Math.round(entryA.median) : null;
        const med2 = hasB && Number.isFinite(entryB.median) ? Math.round(entryB.median) : null;
        const sd1 = hasA && Number.isFinite(entryA.stdDev) ? entryA.stdDev : null;
        const sd2 = hasB && Number.isFinite(entryB.stdDev) ? entryB.stdDev : null;
        const count1 = hasA ? entryA.count : 0;
        const count2 = hasB ? entryB.count : 0;
        ...
        amtSpan1.innerHTML = hasA ? formatAmount(v1, unit) : '<span class="na">n/a</span>';
        td1.appendChild(amtSpan1);
        const sdDiv1 = document.createElement('div');
        sdDiv1.className = 'variance';
        sdDiv1.innerHTML = hasA && sd1 !== null
          ? `(<span title="standard deviation">σ</span>: ${sd1.toFixed(2)})`
          : `(<span title="standard deviation">σ</span>: <span class="na">n/a</span>)`;
        ...
        amtSpan2.innerHTML = hasB ? formatAmount(v2, unit) : '<span class="na">n/a</span>';
        td2.appendChild(amtSpan2);
        const sdDiv2 = document.createElement('div');
        sdDiv2.className = 'variance';
        sdDiv2.innerHTML = hasB && sd2 !== null
          ? `(<span title="standard deviation">σ</span>: ${sd2.toFixed(2)})`
          : `(<span title="standard deviation">σ</span>: <span class="na">n/a</span>)`;
        ...
        if(sentiment !== 'neutral'){
          ...
          if(hasA && v1 !== 0) applyColor(amtSpan1, v1);
          if(hasB && v2 !== 0) applyColor(amtSpan2, v2);
        }
        const meta = {
    ...
    if(DATASET_MODE === 'subset'){
      Object.assign(METRIC_SENTIMENT, {
        'AIExpectedLikelihoodOfBankruptcyBaseStrongerModelGPT':'bad',
        'AIExpectedLikelihoodOfBankruptcyExtendedStrongerModelGPT':'bad',
        'AIExpectedLikelihoodOfBankruptcyBaseStrongerModelGROK':'bad',
        'AIExpectedLikelihoodOfBankruptcyExtendedStrongerModelGROK':'bad',
        'AIExpectedLikelihoodOfBankruptcyAnnualReport':'bad',
        'AIExpectedLikelihoodOfBankruptcyAnnualReportStrongerModelGPT':'bad',
        'AIExpectedLikelihoodOfBankruptcyAnnualReportMoreCharacters':'bad',
        'BankruptcyIndicatingWordsCount':'bad',
        'BankruptcyIndicatingWordsCountAdvanced':'bad',
        'BankruptcyIndicatingWordsCountAdvancedRobust':'bad',
        'SolvencyIndicatingWordsCount':'good',
        'SolvencyIndicatingWordsCountAdvanced':'good',
        'SolvencyIndicatingWordsCountAdvancedRobust':'good',
        'TotalCharactersCount':'neutral'
      });
    }
    ...
    if(DATASET_MODE === 'subset'){
      Object.assign(METRIC_UNITS, {
        ...
        'AIExpectedLikelihoodOfBankruptcyExtendedStrongerModelGPT': '%',
        'AIExpectedLikelihoodOfBankruptcyBaseStrongerModelGROK': '%',
        ...
        'BankruptcyIndicatingWordsCount': 'words',
        'BankruptcyIndicatingWordsCountAdvanced': 'words',
        'BankruptcyIndicatingWordsCountAdvancedRobust': 'words',
        'SolvencyIndicatingWordsCount': 'words',
        'SolvencyIndicatingWordsCountAdvanced': 'words',
        'SolvencyIndicatingWordsCountAdvancedRobust': 'words',
        'TotalCharactersCount': 'characters'
      });
    }

    const METRIC_EXPLANATION_MAP = DATASET_MODE === 'subset' ? {
      ...
      'AIExpectedLikelihoodOfBankruptcyAnnualReport': 'AIExpectedLikelihoodOfBankruptcyAnnualReportExplanation',
      'AIExpectedLikelihoodOfBankruptcyAnnualReportStrongerModelGPT': 'AIExpectedLikelihoodOfBankruptcyAnnualReportStrongerModelGPTExplanation',
      'AIExpectedLikelihoodOfBankruptcyAnnualReportMoreCharacters': 'AIExpectedLikelihoodOfBankruptcyAnnualReportMoreCharactersExplanation'
    } : {};
    ...
    function openCompareDetail(key, v1, med1, sd1, v2, med2, sd2, meta){
      ...
      const hasFirst = meta && Object.prototype.hasOwnProperty.call(meta, 'hasA') ? !!meta.hasA : (v1 !== null && Number.isFinite(v1));
      const hasSecond = meta && Object.prototype.hasOwnProperty.call(meta, 'hasB') ? !!meta.hasB : (v2 !== null && Number.isFinite(v2));
      const td1 = document.createElement('td');
      td1.className = 'amt left';
      const amtSpan1 = document.createElement('span');
      amtSpan1.className = 'amt-val';
      amtSpan1.innerHTML = hasFirst ? formatAmount(v1, unit) : '<span class="na">n/a</span>';
      td1.appendChild(amtSpan1);
      const medDiv1 = document.createElement('div');
      medDiv1.className = 'variance';
      let medSpan1 = null;
      let medLabels1 = [];
      if(hasFirst && med1 !== null && Number.isFinite(med1)){
        ...
        medSpan1 = medDiv1.querySelector('.median-val');
        medLabels1 = Array.from(medDiv1.querySelectorAll('.median-label'));
        medLabels1.forEach(span => span.style.opacity = '0.5');
      } else {
        medDiv1.innerHTML = `<span class="median-label">(median: </span><span class="median-val na">n/a</span><span class="median-label">)</span>`;
        medLabels1 = Array.from(medDiv1.querySelectorAll('.median-label'));
        medLabels1.forEach(span => span.style.opacity = '0.5');
      }
      ...
      const sdDiv1 = document.createElement('div');
      sdDiv1.className = 'variance';
      sdDiv1.innerHTML = hasFirst && sd1 !== null
        ? `(<span title="standard deviation">σ</span>: ${sd1.toFixed(2)})`
        : `(<span title="standard deviation">σ</span>: <span class="na">n/a</span>)`;
      td1.appendChild(sdDiv1);
      const tdSep = document.createElement('td');
      ...
      const amtSpan2 = document.createElement('span');
      amtSpan2.className = 'amt-val';
      amtSpan2.innerHTML = hasSecond ? formatAmount(v2, unit) : '<span class="na">n/a</span>';
      td2.appendChild(amtSpan2);
      const medDiv2 = document.createElement('div');
      medDiv2.className = 'variance';
      let medSpan2 = null;
      let medLabels2 = [];
      if(hasSecond && med2 !== null && Number.isFinite(med2)){
        ...
        medSpan2 = medDiv2.querySelector('.median-val');
        medLabels2 = Array.from(medDiv2.querySelectorAll('.median-label'));
        medLabels2.forEach(span => span.style.opacity = '0.5');
      } else {
        medDiv2.innerHTML = `<span class="median-label">(median: </span><span class="median-val na">n/a</span><span class="median-label">)</span>`;
        medLabels2 = Array.from(medDiv2.querySelectorAll('.median-label'));
        medLabels2.forEach(span => span.style.opacity = '0.5');
      }
      ...
      const sdDiv2 = document.createElement('div');
      sdDiv2.className = 'variance';
      sdDiv2.innerHTML = hasSecond && sd2 !== null
        ? `(<span title="standard deviation">σ</span>: ${sd2.toFixed(2)})`
        : `(<span title="standard deviation">σ</span>: <span class="na">n/a</span>)`;
      td2.appendChild(sdDiv2);
      tr.appendChild(td1);
      ...
      if(hasFirst && hasSecond){
        ...
        noChartMsg.style.opacity = '0.6';
        noChartMsg.style.marginTop = '24px';
        noChartMsg.textContent = 'Not enough data to build comparison charts for this metric.';
        modalContent.appendChild(noChartMsg);
      }
      ...
      if(sentiment !== 'neutral'){
        ...
        if(hasFirst && v1 !== null && v1 !== 0) applyColor(amtSpan1, v1);
        if(hasSecond && v2 !== null && v2 !== 0) applyColor(amtSpan2, v2);
        if(hasFirst && med1 !== null && med1 !== 0){
          applyColor(medSpan1, med1);
          medLabels1.forEach(span => applyColor(span, med1));
        }
        if(hasSecond && med2 !== null && med2 !== 0){
    ...
    function renderYear(cik, year){
      ...
      if(fin){
        ...
        for(const [k,v] of Object.entries(fin)){
          ...
          let amountHTML = formatted !== null ? formatted : val;
          let tooltipText = '';
          if(DATASET_MODE === 'subset'){
            const explanationKey = METRIC_EXPLANATION_MAP[k];
            if(explanationKey && Object.prototype.hasOwnProperty.call(fin, explanationKey)){
              const rawExplanation = fmt(fin[explanationKey]).trim();
              if(formatted === null){
                const parts = [];
                if(val) parts.push(val);
                if(rawExplanation) parts.push(rawExplanation);
                const combined = parts.join(' ').replace(/\s+/g,' ').trim();
                if(combined) tooltipText = combined;
                else tooltipText = rawExplanation || val;
                amountHTML = '<span class="na">n/a</span>';
              } else if(rawExplanation){
                tooltipText = rawExplanation;
              }
            } else if(formatted === null && val){
              amountHTML = '<span class="na">n/a</span>';
              tooltipText = String(val).trim();
            }
          }
          item.innerHTML = `<div class="k">${formatKV(k)}</div><div class="v"><span class="amt">${amountHTML}</span>${pctHtml}</div>`;
          if(tooltipText) item.title = tooltipText;
    ...
    function doSearch(){
      ...
      const ranked = DATA.main
        .filter(r => DATASET_MODE !== 'subset' || financialCIKs.has(String(r.CIK||"").trim()))
