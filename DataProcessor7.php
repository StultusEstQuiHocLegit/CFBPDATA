<?php
// DataProcessor7.php
// Adds weighted textual indicators from annual report HTML filings to financial subset CSV files.
...
define('ADVANCED_COLUMNS', [
    'BankruptcyIndicatingWordsCountAdvanced',
    'SolvencyIndicatingWordsCountAdvanced',
]);
...
logmsg('Starting weighted textual indicator processing from HTML filings…');
...
foreach ($items as $item) {
    ...
    [$negScore, $posScore] = compute_scores_from_text($text, NEGATIVE_WEIGHTS, POSITIVE_WEIGHTS);
    logmsg('  -> weighted bankruptcy words: ' . format_score($negScore) . ', weighted solvency words: ' . format_score($posScore) . '.');
    $processedItems[] = $item + [
...
foreach ($processedItems as $row) {
    ...
    if ($key === '') continue;
    $metrics = [
        'BankruptcyIndicatingWordsCountAdvanced' => (string)$row['neg_score'],
        'SolvencyIndicatingWordsCountAdvanced' => (string)$row['pos_score'],
    ];
    if ($row['source'] === 1) {
...
foreach (ADVANCED_COLUMNS as $col) {
    if (!in_array($col, $finHeader1, true)) $finHeader1[] = $col;
    if (!in_array($col, $finHeader2, true)) $finHeader2[] = $col;
}

[$updated1, $overwrittenCells1] = apply_metrics_to_rows($finRows1, $metricsMap1, ADVANCED_COLUMNS);
[$updated2, $overwrittenCells2] = apply_metrics_to_rows($finRows2, $metricsMap2, ADVANCED_COLUMNS);
...
function fetch_report(string $url): string
{
    ...
    curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (compatible; DataProcessor7/1.0; +https://example.com)');
...
function compute_scores_from_text(string $text, array $negativeWeights, array $positiveWeights): array
{
